...
C++17支持四种类型的锁：`std::lock_guard`/`std::scoped_lock`用于简单场景，`std::unique_lock`/`std::shared_lock`，用于高级场景，如显式锁定或互斥锁解锁。可以在关于锁的章节中了解更多的细节。
