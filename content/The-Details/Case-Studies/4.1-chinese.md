...
首先，第28行检查是否有锁，否则锁和原子操作就没有区别了。所有主流平台上，原子变量都是无锁的。然后，用两种方法计算加和。第33行使用`+=`操作符，第45行使用`fetch_add`方法。单线程情况下，两种方式的性能相差不多；不过，我可以显式地指定`fetch_add`的内存序。关于这点将在下一小节中详细介绍。
...
现在不使用默认的内存序，而使用的是自由语义。因为我们只需要保证，所有求和操作是原子的就好。
...
和预期一样，使用`std::lock_guard`，使用顺序一致的原子变量，或是使用自由语义的原子变量进行求和，在性能方面并没有什么差异。
...
多线程的线程本地求和的速度，大约是单线程求和的两倍。因为线程之间几乎不需要同步，所以在最优的情况下，我认为性能会提高四倍。背后的根本原因是什么？
